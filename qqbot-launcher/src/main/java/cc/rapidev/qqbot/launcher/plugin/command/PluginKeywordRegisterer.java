...
public class PluginKeywordRegisterer implements KeywordRegisterer {
    ...
    public void register(CommandHandlerSet set) {
        set.add(new Keyword("插件列表"), new PluginListCommand(pluginService), Events.C2C_MESSAGE_CREATE);
        set.add(new Keyword("插件详情"), new PluginDetailCommand(pluginService), Events.C2C_MESSAGE_CREATE);
        set.add(new Keyword("启用插件"), new PluginEnableCommand(pluginService), Events.C2C_MESSAGE_CREATE);
        set.add(new Keyword("禁用插件"), new PluginDisableCommand(pluginService), Events.C2C_MESSAGE_CREATE);
    }
